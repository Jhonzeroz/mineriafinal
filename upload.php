...
function InformacionBasica($data, $file) {
    ...
    // Generar HTML con información básica
    $html = "
    <div class='row'>
        <div class='col-md-4'>
            <div class='card small-card shadow-sm mb-3'>
...
// Limpiar la sesión para eliminar datos temporales previos
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    unset($_SESSION['fileData']); // Eliminar datos previos del archivo
    unset($_SESSION['fileSize']); // Eliminar tamaño de archivo previo
    $input = json_decode(file_get_contents('php://input'), true);
    $option = $input['option'] ?? null;
    $action = $input['action'] ?? null;

    // Limpiar la sesión si se envía la acción 'clearSession'
    if ($action === "clearSession") {
        unset($_SESSION['fileData']);
        unset($_SESSION['fileSize']);
        echo json_encode(['status' => 'success', 'message' => 'Datos de la sesión eliminados.']);
        exit;
    }
    ...
    if (isset($_FILES['datafile'])) {
        ...
        $_SESSION['fileData'] = $sheetData;
        $_SESSION['fileSize'] = filesize($file['tmp_name']);
